...
export function generatePythonServer(config: PythonProjectConfig): string {
...
function buildPythonImports(config: PythonProjectConfig): string {
  const lines = [
    '#!/usr/bin/env python3',
    'import os',
    'import asyncio',
...
function buildPythonInit(config: PythonProjectConfig): string {
  const lines = [
    `mcp = FastMCP(`,
    `    name="${config.name}",`,
    `    version="1.0.0",`,
...
function buildPythonTools(config: PythonProjectConfig): string {
  const lines = [
    '',
    '@mcp.tool()',
  ...
  if (config.includeSecurity) lines.push('        "securityLayer": "active",');
  if (config.includeObservability) lines.push('        "metricsEnabled": True,');
