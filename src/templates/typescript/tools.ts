...
export function generateApiTool(config: ProjectConfig): string {
...
export function generateFileTool(config: ProjectConfig): string {
  const securityImport = config.includeSecurity
    ? `import { securityLayer } from '../security/archestraSecurity.js';`
    : '';
  const observabilityImport = config.includeObservability
...
export function generateDatabaseTool(config: ProjectConfig): string {
  const observabilityImport = config.includeObservability
    ? `import { recordToolCall } from '../observability/metrics.js';
import { logger } from '../observability/logger.js';`
    : '';
...
export function generateIndexFile(config: ProjectConfig): string {
