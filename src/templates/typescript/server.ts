...
export function generateServerFile(config: ProjectConfig): string {
...
function buildImports(config: ProjectConfig): string {
  const lines = [
...
function buildInitCode(config: ProjectConfig): string {
  const lines = [
    'dotenv.config();',
    '',
...
function buildToolRegistration(config: ProjectConfig): string {
  const lines = [
    '// Register tools',
    "mcp.addTool({",
    "  name: 'example_tool',",
